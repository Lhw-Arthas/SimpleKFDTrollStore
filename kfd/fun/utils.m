...
//    NSLog(path);
    uint64_t var_tmp_vnode = getVnodeAtPathByChdir(path);
...
uint64_t createFolderAndRedirect3(char *path) {
    NSString *mntPath = [NSString stringWithFormat:@"%@%@", NSHomeDirectory(), @"/Documents/mounted1"];
    uint64_t var_tmp_vnode = getVnodeAtPathByChdir(path);
    printf("[i] vnode: 0x%llx\n", var_tmp_vnode);
    uint64_t orig_to_v_data = createFolderAndRedirect(var_tmp_vnode, mntPath);
    return orig_to_v_data;
...
uint64_t overwriteFileVarHelp(char* to, char* from) {
    funVnodeOverwriteFileUnlimitSize(to, from);
    return 0;
}
